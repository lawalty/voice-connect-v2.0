...
for (const provider of ['browser', 'fish'] as const) {
  test(`${provider}: orb and messenger share typed speech, draft, voice capture, and agent mute`, async ({ page, context }, info) => {
    ...
    await composer.fill('A typed reply from messenger.'); await page.getByRole('button', { name: 'Send message', exact: true }).click();
    ...
    await expect(page.getByRole('log').getByText('A typed reply from messenger.', { exact: true })).toBeVisible();
