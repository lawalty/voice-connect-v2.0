...
| Input continuity | Switching Orb/Messenger, focusing the composer, and typed Send preserve capture and the native conversation. Typed and spoken drafts remain independent. Explicit Edit as text and Messenger Auto off stop capture and preserve unsent speech without stopping the current reply. Late recognition after an explicit stop cannot submit stale speech. Delayed voice/text receipts cannot erase newer typing; a second completed voice turn during pending delivery pauses capture and remains in the composer. Composer initialization cannot overwrite fresh typing because input waits for conversation selection. |
