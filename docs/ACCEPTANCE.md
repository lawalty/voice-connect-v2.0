...
| Input continuity | Switching Orb/Messenger, focusing the composer, and typed Send preserve capture and the native conversation. Messenger shows live transcription in the message field and combines typed additions into that same automatically submitted turn. Recognition revisions cannot overwrite typed additions. The combined unsent draft survives Auto off and refresh; Orb-only input keeps its existing separate transcript. Explicit Edit as text and Messenger Auto off stop capture and preserve unsent speech without stopping the current reply. Late recognition after an explicit stop cannot submit stale speech. Delayed voice/text receipts cannot erase newer typing; a second completed voice turn during pending delivery pauses capture and remains in the composer. Composer initialization cannot overwrite fresh typing because input waits for conversation selection. |
