...
export default function App() {
  ...
  useEffect(() => {
    ...
    setMessages([]); updateHeard(''); setActiveTurn(null); setActivity(''); spoken.current.clear(); sequences.current.clear(); completed.current.clear(); cancelled.current.clear(); speakingTurn.current = '';
  ...
  }, [abort, refreshHistory, enterTextMode, updateDraft]); submitRef.current = text => submit(text, 'voice');

  async function startVoice() {
    if (!conversationId || !engine.current) return; if (heard.trim()) updateDraft(value => value ? `${value}\n${heard.trim()}` : heard.trim()); setNotice(''); voiceRef.current = true; setVoiceActive(true); setMuted(false);
